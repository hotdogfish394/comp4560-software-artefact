...
# change pass forward to include v
class Net(nn.Module):
    def __init__(self, in_channels=3, conditional_dim=0):
        super(Net, self).__init__()
        self.condition_dim = conditional_dim
        self.vit = ViTModel.from_pretrained('google/vit-base-patch16-224-in21k',num_channels=in_channels,ignore_mismatched_sizes=True)
        self.fc = nn.Linear(768 + self.condition_dim, 2)
    ...
    def forward(self, x, v):
        ...
        x = torch.cat((x[:, 0, :], v), dim=1)
        x = self.fc(x)
        
        return x

# define the LightningModule
class LitViTV(pl.LightningModule):
